...
import { delikAlaniFonksiyonu } from "./abajur-delik.mjs";

/** Geometri sürümü. Şekli etkileyen her değişiklikte artır. */
export const GEO_SURUM = "3.4";
...
export function abajurGeometrisi(p, kalite = KALITE.uretim) {
  ...
  for (let r = 0; r <= R; r++) {
    ...
    for (let c = 0; c < N; c++) {
      ...
      if (F) {
        let fv = delik.f(th, u);
        // Kesişim tam köşeye denk gelirse sıfır uzunlukta kenar ve çift
        // duvar oluşuyor, ağ açılıyordu. Köşeyi sıfırdan uzaklaştır:
        // 0.1 mikron kaydırma geometride ölçülemez, ağı sağlamlaştırır.
  ...
  if (!delik) {
    ...
    const ayniDugum = (a, b) => {
      const i = a * 3, j = b * 3;
      return Math.round(pos[i] * 1000) === Math.round(pos[j] * 1000)
        && Math.round(pos[i + 1] * 1000) === Math.round(pos[j + 1] * 1000)
        && Math.round(pos[i + 2] * 1000) === Math.round(pos[j + 2] * 1000);
    };
  ...
  geo.userData.bolum = { N, R, OUT: 0, IN: 1, delikli: Boolean(delik), delikBilgi: delik?.bilgi || null };
  return geo;
}
