...
function montajGeometrisi(p) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    kutu.applyMatrix4(
      new THREE.Matrix4()
        .makeRotationY(-th)
        .setPosition(((rIc + rDis) / 2) * Math.cos(th), H - hh / 2, ((rIc + rDis) / 2) * Math.sin(th))
    );
    kollar.push(kutu);
  }
...
export function uret(girdi) {
  ...
  const vazo = p.montaj !== "boyun";
  const duvarSayisi = Math.max(1, Math.round(p.cidar / 0.42));

  const ad = `abajur_${p.profil}_${p.altCap}x${p.yukseklik}_${p.desen}_${p.malzeme.replace(/\s/g, "")}`;
  ...
  const isEmri = {
    ...
    dilimleyici: vazo
      ...
      : {
          ...
          altKat: "3",
          katYuksekligi: "0,2 mm",
          not: "Boyun ve kollar gövdeyle çakışan ayrı katılar; dilimleyici birleştirir",
        },
    montaj:
      p.montaj === "boyun"
        ? `${p.duyTipi} entegre boyun · boğaz Ø${p.bogazCap} mm · ${p.kolSayisi} kol`
        : "Duy halkası ile sıkıştırılır",
