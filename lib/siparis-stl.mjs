...
import { DELIK_DESENLERI } from "./abajur-delik.mjs";
import {
  GEO_SURUM,
  TABLA,
  PAY,
...
export function uret(girdi, secenek = {}) {
  ...
  const uyarilar = [...(secenek.ekUyarilar || [])];
  ...
  /* Delik deseni baskı davranışını değiştirir: baklava delikler sivri
     bittiği için desteksiz kapanır, altıgen ve organik hücrelerin tavanı
     kısa köprüler oluşturur. Üretimde bunu bilmek gerekiyor. */
  const delikli = p.delik && p.delik !== 'yok' && DELIK_DESENLERI.includes(p.delik);
  const delikAdi = { elmas: 'Elmas kafes', petek: 'Petek', organik: 'Organik hücre' }[p.delik] || '';
  if (delikli && p.cubukKalinlik < 2) {
    uyarilar.push(
      `Çubuk kalınlığı ${p.cubukKalinlik} mm — 2 mm altı kafes kırılgan olur, taşımada zarar görebilir.`
    );
  }
  ...
  // kafes deseni dosya adında görünsün: aynı gövdeden farklı ürünler çıkıyor
  const kafesEki = delikli ? `_${p.delik}` : '';
  const ad = `abajur_${p.profil}_${Math.round(p.altCap)}x${Math.round(p.yukseklik)}_${p.desen}${kafesEki}_${p.duyTipi}_${String(
    p.malzeme
  ).replace(/\s/g, "")}`;
  ...
  const isEmri = {
    ...
    dilimleyici: {
      ...
      // Dolgu yalnızca boyun ve taşıyıcıların içinde maddeleşir; gövde duvarı
      // tamamen konturdan oluşur. Bu parçalar duyun ve ampulün yükünü
      // taşıdığı için %30 seçildi — toplam süreye etkisi birkaç dakika.
      dolgu: "%30 gyroid — yalnızca boyun ve taşıyıcı içinde oluşur, gövde duvarında dolgu yoktur",
      ustKat: "4",
      altKat: "4",
      ...
      ...(delikli ? {
        delikKoprusu: p.delik === 'elmas'
          ? "Baklava delikler sivri kapandığı için köprü yok, destek gerekmez"
          : "Hücre tavanlarında kısa köprüler var; destek gerekmez, köprü hızını düşürmek yüzeyi iyileştirir"
      } : {}),
      not: "Boyun rozeti ve yaprak taşıyıcılar gövdeyle çakışan kapalı katılardır; dilimleyici birleştirir",
    },
    delikDeseni: delikli
      ? `${delikAdi} · çubuk ${p.cubukKalinlik} mm · hücre ~${p.delikBoyu} mm${p.delik === 'organik' ? ` · tohum ${p.delikTohum}` : ''}`
      : 'Yok — kapalı gövde',
