import { GEO_SURUM, uret } from './siparis-stl.mjs';
...
const DEFAULTS = {
  ...
  kolKalinlik: 5, kelvin: 2700, adet: 1
};
...
const RANGE = {
  ...
  dalgaSayisi: [1, 20], cidar: [0.6, 3], boyunH: [6, 30], kolSayisi: [2, 8],
  kolKalinlik: [3, 12], kelvin: [2200, 6000], adet: [1, 20]
};
...
export function normalizeAbajurConfig(input = {}) {
  ...
  const bogazRange = duyTipi === 'E14' ? [22, 38] : [34, 52];
  const bogaz = Number(input.bogazCap);
  return {
    paket: option(input.paket, 'paket'),
    duyTipi,
    profil: option(input.profil, 'profil'),
    altCap: numberInRange(input.altCap, 'altCap'),
    ustCap: numberInRange(input.ustCap, 'ustCap'),
    ...
    montaj: option(input.montaj, 'montaj'),
    bogazCap: Number.isFinite(bogaz) ? Math.max(bogazRange[0], Math.min(bogazRange[1], bogaz)) : DEFAULTS.bogazCap,
    boyunH: numberInRange(input.boyunH, 'boyunH'),
    kolSayisi: Math.round(numberInRange(input.kolSayisi, 'kolSayisi')),
    kolKalinlik: numberInRange(input.kolKalinlik, 'kolKalinlik'),
