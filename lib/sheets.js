...
const ORDER_HEADERS = ['siparis_no','tarih','musteri','telefon','email','adres','not','urunler','toplam','durum'];
...
async function ensureSheet(sheets, title, headers) {
  ...
  if (!r.data.values?.[0]?.length) await sheets.spreadsheets.values.update({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${title}!A1`, valueInputOption: 'RAW', requestBody: { values: [headers] } });
}
...
export async function appendOrder(o) {
  ...
  const row = [safeCell(o.orderNo, 120), new Date().toISOString(), safeCell(o.name, 120), safeCell(o.phone, 30), safeCell(o.email || '', 160), safeCell(o.address, 500), safeCell(o.note || '', 500), safeCell(o.items.map(i => `${i.name} x${i.quantity}`).join(' | '), 2500), Number(o.total), 'Yeni'];
  await sheets.spreadsheets.values.append({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${ORDERS}!A:J`, valueInputOption: 'USER_ENTERED', requestBody: { values: [row] } });
  return row;
}
export async function getDashboard() {
  ...
  return { totalOrders: rows.length, todayOrders, monthOrders: monthOrders.length, totalRevenue, monthRevenue: monthOrders.filter(r => (r[9] || 'Yeni') !== 'İptal').reduce((s,r)=>s+Number(r[8]||0),0), pending: rows.filter(r => ['Yeni','Onaylandı','Hazırlanıyor','Kargoya Hazır'].includes(r[9] || 'Yeni')).length };
}
export async function getOrders() {
  const sheets = client();
  await ensureSheet(sheets, ORDERS, ORDER_HEADERS);
  const r = await sheets.spreadsheets.values.get({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${ORDERS}!A2:J` });
  return (r.data.values || []).reverse().slice(0,100).map(r => ({ orderNo:r[0], date:r[1], name:r[2], phone:r[3], email:r[4], address:r[5], note:r[6], items:r[7], total:Number(r[8]||0), status:r[9]||'Yeni' }));
}

export async function updateOrderStatus(orderNo, status) {
  const allowed = ['Yeni','Onaylandı','Hazırlanıyor','Kargoya Hazır','Kargolandı','Tamamlandı','İptal'];
