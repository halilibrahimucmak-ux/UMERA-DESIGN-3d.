...
const ABAJUR_FIYAT_SAYFA = 'AbajurFiyat';
const ABAJUR_FIYAT_HEADERS = ['anahtar','deger','guncelleme'];
...
/* ------------------------- ABAJUR FİYAT TARİFESİ -------------------------
   Tarife anahtar/değer satırları halinde tutulur (ör. "filament.PLA" = 780).
   Böylece yeni bir kalem eklendiğinde sayfanın yapısını değiştirmek
   gerekmez ve eski kayıtlar bozulmaz. */

/** Kaydedilmiş tarife alanlarını { "filament.PLA": 780, kar: 55 } olarak döndürür. */
export async function getAbajurFiyat() {
  const sheets = client();
  await ensureSheet(sheets, ABAJUR_FIYAT_SAYFA, ABAJUR_FIYAT_HEADERS);
  const r = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
    range: `${ABAJUR_FIYAT_SAYFA}!A2:C`
  });
  const kayit = {};
  for (const row of r.data.values || []) {
    const anahtar = String(row[0] || '').trim();
    if (!anahtar) continue;
    const sayi = Number(String(row[1] ?? '').replace(',', '.'));
    if (Number.isFinite(sayi)) kayit[anahtar] = sayi;
  }
  return kayit;
}

/** Verilen alanları yazar; gönderilmeyen alanlara dokunmaz. */
export async function saveAbajurFiyat(alanlar) {
  const sheets = client();
  await ensureSheet(sheets, ABAJUR_FIYAT_SAYFA, ABAJUR_FIYAT_HEADERS);
  const r = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.GOOGLE_SHEET_ID,
  ...
  for (const [anahtar, deger] of Object.entries(alanlar)) {
    const idx = rows.findIndex(row => String(row[0] || '').trim() === anahtar);
    if (idx >= 0) {
      const rowNo = idx + 2;
      guncellemeler.push({
        range: `${ABAJUR_FIYAT_SAYFA}!A${rowNo}:C${rowNo}`,
        values: [[anahtar, Number(deger), simdi]]
      });
    } else {
      yeniSatirlar.push([anahtar, Number(deger), simdi]);
    }
  }
  ...
  if (guncellemeler.length) {
    await sheets.spreadsheets.values.batchUpdate({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      requestBody: { valueInputOption: 'USER_ENTERED', data: guncellemeler }
    });
  }
  if (yeniSatirlar.length) {
    await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: `${ABAJUR_FIYAT_SAYFA}!A:C`,
      valueInputOption: 'USER_ENTERED',
      requestBody: { values: yeniSatirlar }
    });
  }
  return getAbajurFiyat();
}
