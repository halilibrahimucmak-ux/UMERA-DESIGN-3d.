...
const PRODUCT_HEADERS = ['id','urun','kategori','fiyat','stok','gorsel','aciklama','aktif','olusturma_tarihi','gorseller'];
...
export async function getProducts() {
  ...
  const r = await sheets.spreadsheets.values.get({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${PRODUCTS}!A2:J` });
...
export async function createProduct(p) {
  ...
  const row = [id(), p.name, p.category, Number(p.price), Number(p.stock), images[0]?.url || '', p.description || '', 'true', new Date().toISOString(), images.length ? safeCell(JSON.stringify(images), 12000) : ''];
  await sheets.spreadsheets.values.append({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${PRODUCTS}!A:J`, valueInputOption: 'USER_ENTERED', requestBody: { values: [row] } });
  return row;
}
export async function updateProduct(productId, p) {
  ...
  const rowNo = idx + 2;
  const images = gorselleriDuzenle(p);
  const row = [productId, p.name, p.category, Number(p.price), Number(p.stock), images[0]?.url || '', p.description || '', String(p.active !== false), rows[idx][8] || new Date().toISOString(), images.length ? safeCell(JSON.stringify(images), 12000) : ''];
  await sheets.spreadsheets.values.update({ spreadsheetId: process.env.GOOGLE_SHEET_ID, range: `${PRODUCTS}!A${rowNo}:J${rowNo}`, valueInputOption: 'USER_ENTERED', requestBody: { values: [row] } });
  return row;
}
